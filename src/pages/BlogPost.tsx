...
export default function BlogPost(): JSX.Element {
  ...
  return (
    <main>
      ...
      <PageHero eyebrow={post.category} title={post.title} description={post.executiveSummary} breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Insights', href: '/insights' }, { label: post.title }]} aside={<p className="text-sm leading-7">Published {new Date(`${post.datePublished}T12:00:00`).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}<br />General information, not legal advice.</p>} />
      <Container className="grid gap-14 py-16 sm:py-20 lg:grid-cols-[minmax(14rem,0.45fr)_minmax(0,1fr)] lg:gap-24 lg:py-28">
        <aside><p className="eyebrow">Key takeaways</p><ul className="mt-6 border-t border-ink/20">{post.keyTakeaways.map((item, index) => <li key={item} className="grid grid-cols-[2.5rem_1fr] gap-3 border-b border-ink/20 py-4 text-sm leading-7 text-muted"><span className="font-display text-xl text-gold-dark">0{index + 1}</span>{item}</li>)}</ul></aside>
