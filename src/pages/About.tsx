...
export default function About(): JSX.Element {
  return <main>
    ...
    <PageHero visual="city" eyebrow="The firm" title="About Murray Legal" description="A focused legal practice for clients navigating consequential business, property, dispute, creative, and private matters." breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'About' }]} />
    ...
    <section className="section-shell bg-ivory"><Container><p className="eyebrow">Working principles</p><div className="mt-10 grid border-l border-t border-ink/15 lg:grid-cols-3">{principles.map(([title, body]) => <article key={title} className="border-b border-r border-ink/15 p-7 sm:p-9"><span className="block h-1 w-10 bg-gold" aria-hidden="true" /><h2 className="mt-10 font-display text-3xl text-ink">{title}</h2><p className="mt-5 text-sm leading-7 text-muted">{body}</p></article>)}</div></Container></section>
