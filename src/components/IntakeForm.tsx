...
export default function IntakeForm(): JSX.Element {
  ...
  if (status === 'success') {
    return <div className="border border-gold/40 bg-paper p-8 sm:p-10" role="status"><CheckCircle2 className="text-gold-dark" size={34} aria-hidden="true" /><h2 className="mt-6 font-display text-4xl text-ink">Your request has been received.</h2><p className="mt-5 text-base leading-8 text-muted">Murray Legal will review the information for fit, conflicts, and jurisdiction. No attorney-client relationship exists unless the firm confirms an engagement in writing.</p><button className="btn-secondary mt-7" type="button" onClick={() => setStatus('idle')}>Submit another request</button></div>;
  }
  ...
  return <form className="border border-ink/15 bg-paper p-6 sm:p-9" onSubmit={submit} noValidate>
    ...
    <div className="mt-8 grid gap-x-6 gap-y-5 sm:grid-cols-2">
      ...
      <label className="text-sm font-semibold text-ink sm:col-start-2 sm:row-start-1" htmlFor="email">Email address *</label><div className="sm:col-start-2 sm:row-start-2"><input className="field !mt-0" type="email" autoComplete="email" {...inputProps('email')} /><ErrorText name="email" /></div>
