...
export default function LegalDocument({ title, description, updated, sections }: { title: string; description: string; updated: string; sections: LegalSection[] }): JSX.Element {
  return <>
    <PageHero eyebrow="Legal information" title={title} description={description} breadcrumbs={[{ label: 'Home', href: '/' }, { label: title }]} aside={<p className="text-sm leading-7">Last updated<br /><span className="text-paper">{updated}</span></p>} />
